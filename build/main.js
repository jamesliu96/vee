...
addEventListener('load', async () => {
    const go = new Go();
    const { instance } = await WebAssembly.instantiateStreaming(fetch('vee.wasm'), go.importObject);
    go.run(instance);
    ...
    button.addEventListener('click', async () => {
        ...
        if (data) {
            const output = await Vee.faux(new Uint8Array(data), new TextEncoder().encode(key.value));
